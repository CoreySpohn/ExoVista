...
from ExoVista import Settings, generate_disks
...
def read_solarsystem(settings, system_file="example_system.dat"):
    ...
    while True:
        ...
        for label in alias:
            if tag.lower().startswith(label.lower()):
                if label.lower == "ra" and tag.lower().startswith("rad"):
                    continue
                tag = alias[label]
                if tag in strlist:
                    s[tag] = val
    ...
    if s["Vmag"].loc[0] == 0.0 and s["M_V"].loc[0] != 0.0:
        s["Vmag"] = s["M_V"] - 5 + 5 * np.log10(s["dist"])  # apparent V band mag
    if s["Bmag"].loc[0] == 0.0:
        s["Bmag"] = s["Vmag"] + BmV  # apparent B band mag
    s["angdiam"] = 0.465 * s["rstar"] / (s["dist"] / 10.0)
    ...
    while i < maxnplanets:
        ...
        for tag in pllist:
            if tag == "albedo":
                albedo_files.append(l[pllist.index(tag)])
            elif tag in pllabel:
                j = pllabel.index(tag)
    ...
    while i < len(disk):
        try:
            ...
            break
        if len(l) == 0 or l.split()[0] == "Settings":
            break

        l = l.split()
        for tag in dlist:
            if tag == "composition":
                composition = [l[dlist.index(tag)]]
            elif tag in dlabel:
                j = dlabel.index(tag)
        ...
        if i == 2:
            tcoll = torbit / (
                4
                * np.pi
                * (disk[1, dlabel.index("nzodis")] + disk[2, dlabel.index("nzodis")])
                * 1e-7
            )  # yr
        else:
            tcoll = torbit / (4 * np.pi * disk[i, dlabel.index("nzodis")] * 1e-7)  # yr
    ...
    while True:
        ...
        val = l[1]
        if tag == "npix":
            setattr(settings, tag, int(val))
        elif tag in vars(settings):
            setattr(settings, tag, float(val))
