...
def generate_disks(stars, planets, settings, nexozodis=None, rand_components=False):
    ...
    if settings.ncomponents < mincomponents or settings.ncomponents > maxcomponents:
        print(
            "ERROR: Maximum number of disk components is set to {0:d}-{1:d}".format(
    ...
    if nexozodis is not None:
        ...
        occ_rate_path = resources.files("ExoVista").joinpath(
            "data/nominal_maxL_distribution-Dec2019.fits"
        )
        with fits.open(occ_rate_path) as data:
            nexozodi_levels = pd.DataFrame(data[0].data)
    ...
    disks[:, :, dlabel.index("longnode")] = 0.0
    disks[:, :, dlabel.index("i")] = 0.0
    ...
    maxfac = np.log10(
        settings.density_ratio
    )  # outer disks must be within some density factor of inner disk
    minfac = np.log10(1.0 / settings.density_ratio)
    for i in range(0, nstars):
        if ndisks[i] > 1:
            for icomp in range(1, ndisks[i]):
                disks[:, icomp, dlabel.index("nzodis")] = (
                    10.0 ** (rng.random(nstars) * (maxfac - minfac) + minfac)
                    * disks[:, 0, dlabel.index("nzodis")]
                )
    ...
    # lqqdir = "lqq_files/"
    lqqdir = resources.files("ExoVista").joinpath("data/lqq_files")
    complist = []
    ...
    for i in range(0, nstars):
        ...
        hill_radius = (
            p[:, pllabel.index("a")]
            * (1.0 - p[:, pllabel.index("e")])
            * (p[:, pllabel.index("M")] / (3 * s["mass"] * 333000.0)) ** (1.0 / 3.0)
        )  # AU
        ...
        j = [
            ...
            if hinner[j] > 0 and p[j, pllabel.index("M")] > 1.0e-3
        ]
        hinner = hinner[j]
        houter = houter[j]
        ...
        gouter = np.append(hinner, 1.0e20)
        ...
        for j in range(0, ndisks[i]):
            ...
            disks[i, j, dlabel.index("hor")] = (
                rng.random() * (settings.hor_max - settings.hor_min) + settings.hor_min
            )
            disks[i, j, dlabel.index("g0")] = (
                rng.random() * (0.995 - 0.8) + 0.8
            )  # most forward scatt term
            disks[i, j, dlabel.index("g1")] = (
                rng.random() * (0.8 - 0.35) + 0.35
            )  # medium forward scatt term
            disks[i, j, dlabel.index("g2")] = (
                rng.random() * (0.35 + 0.3) - 0.3
            )  # least forward scatt term
            disks[i, j, dlabel.index("w0")] = (
                rng.random() * (0.8 - 0.4) + 0.4
            )  # most forward scatt term
            minw = 0.1
            maxw = 1.0 - disks[i, j, dlabel.index("w0")]
            disks[i, j, dlabel.index("w1")] = (
                rng.random() * (maxw - minw) + minw
            )  # medium forward scatt term
            disks[i, j, dlabel.index("w2")] = (
                1.0 - disks[i, j, dlabel.index("w0")] - disks[i, j, dlabel.index("w1")]
            )  # least forward scatt term
            ...
            while (
                (dror < settings.dror_min)
                or (r0 < settings.r_min[j] or settings.r_max[j] < r0)
                and n < 50
            ):
            ...
            k = np.array(
                [
                    ...
                    if p[jj, pllabel.index("a")] < disks[i, j, dlabel.index("r")]
                    and p[jj, pllabel.index("M")] > settings.rinner_mass_threshold
                ]
            )
            if len(k) > 0:
                disks[i, j, dlabel.index("rinner")] = min(
                    (
                        1.1
                        * np.max(
                            p[k, pllabel.index("a")] * (1 + p[k, pllabel.index("e")])
                        )
                    ),
                    disks[i, j, dlabel.index("r")],
                )

            # If the while loop failed to find a valid r, set density to zero
            if dror < settings.dror_min:
                disks[i, j, dlabel.index("nzodis")] = 0.0

            # Now that we have the locations we can determine collision rates...
            beta = 0.5
            alpha = (
                beta * grav_const * s["mass"] / c
            )  # quantity related to PR drag to be used later
            tpr = disks[i, j, dlabel.index("r")] ** 2 / (4 * alpha)  # yr
            torbit = np.sqrt(disks[i, j, dlabel.index("r")] ** 3 / s["mass"])  # yr
            tcoll = torbit / (
                4 * np.pi * disks[i, j, dlabel.index("nzodis")] * 1e-7
            )  # yr
