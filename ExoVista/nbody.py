...
def nbody(cartin, GM, R, istar, curr_time, desired_time):
    ...
    while t < desired_time:
        ...
        for i in range(0, n_planets - 1):
            transmaster[i].append(translist[i])
...
def detect_transits(R, istar, state):
    n_planets = len(R)
    sini = np.sin(istar * np.pi / 180.0)
    cosi = np.cos(istar * np.pi / 180.0)
    ...
    for i_pl in range(1, n_planets):
        ...
        sep[i_pl - 1] = max(
            1, np.sqrt((xpl - xstar) ** 2 + (ypl - ystar) ** 2) / (R[0] + R[i_pl])
        )
        vxy = np.sqrt(vxpl**2 + vypl**2) / (R[0] + R[i_pl])
        ttrans[i_pl - 1] = sep[i_pl - 1] / vxy

        if sep[i_pl - 1] == 1 and zpl > 0:
            translist[i_pl - 1] = 1
        elif sep[i_pl - 1] == 1 and zpl < 0:
            translist[i_pl - 1] = -1
