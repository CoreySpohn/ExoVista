...
from ExoVista.constants import maxnplanets, pllabel
...
def radius_to_mass(R):
    ...
    M[i] = (R[i] / 1.008) ** (1.0 / 0.279)
    i = [j for j in range(0, len(R)) if R[j] > 1.008 * 2.04**0.279]
    M[i] = (R[i] / (1.008 * 2.04**0.279)) ** (1.0 / 0.589) * 2.04
    i = [
        j for j in range(0, len(R)) if R[j] > 12.22
    ]  # R(M) is not monotonic in this range.
    M[i] = -1.0
...
def generate_planets(
    ...
    cossysimin = np.cos(settings.sysimin * np.pi / 180.0)
    cossysimax = np.cos(settings.sysimax * np.pi / 180.0)
    ...
    if usebins and settings.randrad:
        print(
            "Error: usebins keyword is not compatible with the randomized mass-radius relation (randrad)"
        )
        return
    ...
    stars["PA"] = (
        rng.random(nstars) * (settings.sysPAmax - settings.sysPAmin) + settings.sysPAmin
    )

    stars["I"] = (
        np.arccos((rng.random(nstars) * (cossysimin - cossysimax) + cossysimax))
        * 180.0
        / np.pi
    )
    ...
    occrates, medge, aedge, mbins, abins = load_occurrence_rates(
        bound=bound, mass=True, usebins=usebins
    )
    ...
    if not usebins:
        ...
        for i in range(0, len(mbound) - 1):
            try:
                mmin = np.where(mbins >= mbound[i])[0][0]
            except:
                mmin = 0
            try:
                mmax = np.where(mbins >= mbound[i + 1])[0][0]
            except:
                mmax = len(mbins)

            for j in range(0, len(abound) - 1):
                try:
                    amin = np.where(abins >= abound[i, j])[0][0]
                except:
                    amin = 0
                try:
                    amax = np.where(abins >= abound[i, j + 1])[0][0]
                except:
                    amax = len(abins)

                expected_in_bin = int(
                    np.round(np.sum(occrates[amin:amax, mmin:mmax]) * nstars)
                )
                if expected_in_bin == 0:
                    continue
                ...
                for ii in range(amin, amax):
                    for jj in range(mmin, mmax):
                        randgrid[ii, jj] = rng.random(nstars)
                        expected[ii, jj] = len(
                            np.where(randgrid[ii, jj] < occrates[ii, jj])[0]
                        )
                ...
                if diff == 0:
                    ...
                    for kk in range(0, diff):
                        ind = np.unravel_index(
                            np.argmax(randgrid[amin:amax, mmin:mmax, :]),
                            (amax - amin, mmax - mmin, nstars),
                        )
                        ind2 = np.argmax(randgrid[amin:amax, mmin:mmax, :])
                        randgrid[amin + ind[0], mmin + ind[1], ind[2]] = 0.0
                elif diff < 0:
                    while diff < 0:
                        ind = np.unravel_index(
                            np.argmin(randgrid[amin:amax, mmin:mmax, :]),
                            (amax - amin, mmax - mmin, nstars),
                        )
                        if (
                            randgrid[amin + ind[0], mmin + ind[1], ind[2]]
                            < occrates[amin + ind[0], mmin + ind[1]]
                        ):
                            diff += 1
                        randgrid[amin + ind[0], mmin + ind[1], ind[2]] = 1.0
    ...
    hillsphere_flag = np.full(
        nstars, True
    )  # improves run time efficiency: calculate new planets only if True.
    nplanets = 0
    n_nochange = 0
    jj = 0
    stars_with_Earths = 0
    ...
    while nplanets < nexpected and n_nochange < 50 and jj < 200:
        ...
        if dnp <= 0:
            n_nochange += 1  # Ends loop if no increase for 50 iterations.
        else:
    ...
    for i in range(0, nstars):
        for j in range(0, maxnplanets):
            if plorb[i, j, pllabel.index("R")] == 0.0:
                plorb[i, j, pllabel.index("a")] = 0.0
    ...
    print(
        "{0:0.1f} planets expected from occurrence rates".format(
            np.sum(occrates) * nstars
        )
    )
    if not nomcdraw:
        print(
            "{0:0.1f} planets expected from Monte Carlo draw of occurrence rates".format(
                nexpected
            )
        )
    ...
    if addearth:
        for i in range(0, nstars):
            ...
            plorb[i, inew] = np.array(
                [1.0e-6, 1.0, anew, 0.00, 0.0005, -11.261, 114.208, -102.947]
            )
...
def load_occurrence_ratesMA(filename, subdivide=1, usebins=False):
    ...
    if usebins:
        medge = np.append(medge, radius_to_mass(rbound[1:-1]))
    medge.sort()
    ...
    if usebins:
        aedge = np.append(aedge, np.unique(abound[:, 1:-1].flatten()))
    aedge.sort()
...
def add_planets(
    ...
    nstars = len(stars)
    hillsphere_flag = np.full(
        len(hillsphere_flag), False
    )  # reset this flag to no recalculation for all stars

    # First make a 2D histogram of a and M
    temp_a = []
    temp_M = []
    for i in range(0, nstars):
        temp_alist = plorb[i, :, pllabel.index("a")] / np.sqrt(
            stars["Lstar"].loc[i]
        )  # undo the sqrt(L) factor
    ...
    h, xedges, yedges = np.histogram2d(
        np.log(temp_a[j]), np.log(temp_M[j]), [realbinsx, realbinsy]
    )
    ...
    temp_planets[:, pllabel.index("a")] = 1.0e20

    temp_planets[:, pllabel.index("longnode")] = rng.random(nplanets) * 360.0
    temp_planets[:, pllabel.index("argperi")] = rng.random(nplanets) * 360.0
    temp_planets[:, pllabel.index("meananom")] = rng.random(nplanets) * 360.0
    temp_planets[:, pllabel.index("e")] = settings.emin + rng.random(nplanets) * (
        settings.emax - settings.emin
    )
    temp_planets[:, pllabel.index("i")] = settings.imin + rng.random(nplanets) * (
        settings.imax - settings.imin
    )
        ...
        for j in range(0, len(ora_array) - 1):
            if nplanets_histo[j, i] > 0:
                ...
                for k in range(0, ntemp):
                    tempM = np.exp(
                        rng.random() * (lorM_array[i + 1] - lorM_array[i])
                        + lorM_array[i]
                    )
    ...
    for i in range(0, len(orM_array) - 1):
        for j in range(0, len(ora_array) - 1):
            if nplanets_histo[j, i] > 0:
                ...
                for k in range(0, ntemp):
                    tempa = np.exp(
                        rng.random() * (lora_array[j + 1] - lora_array[j])
                        + lora_array[j]
                    )
    ...
    temp_planets[:, pllabel.index("R")] = mass_to_radius(
        temp_planets[:, pllabel.index("M")],
        rng,
        temp_planets[:, pllabel.index("a")],
        randrad,
    )
    ...
    for i in range(0, nstars):
        ...
        if len(ilist) == 0:
            ...
            pold = [
                plorb[i, j]
                for j in range(0, maxnplanets)
                if plorb[i, j, pllabel.index("R")] > 0
            ]
            if len(pold) == 0:
                pold = [plorb[i, 0]]
            temp_planets2 = np.concatenate(
                (np.array(temp_planets[ilist]), np.array(pold))
            )
...
def remove_unstable_planets(stars, plorb, hillsphere_flag, force_earth=False):
    ...
    for i in range(0, nstars):
        if hillsphere_flag[i]:
            threestarmass_EM = (
                3 * stars["mass"].loc[i] * 333000.0
            )  # star mass in Earth masses
            ...
            while plorb[i, j + 1, pllabel.index("R")] > 0:
                ...
                if mutual_hill_sphere < mhs_factor:
                    ...
                    plorb[i, j0, pllabel.index("a")] = 1.0e20

                    l = np.argsort(
                        plorb[i, :, pllabel.index("a")]
                    )  # re-sort to get rid of deleted planet
                    plorb[i] = plorb[i, l]
                    if j > 0:
                        j -= 1  # there's now one fewer planet; re-check against the next planet in if there is one

                else:
                    j += 1  # if the planet pair was okay, increase j
                ii += 1
                if j == maxnplanets - 1:
                    break  # Don't go past the end of the planets list
    return plorb
...
def assign_albedo_file(stars, plorb, rng):
    ...
    for i in range(0, nstars):
        plalbedo.append([])
        nplanets = len(np.where(plorb[i, :, pllabel.index("R")] > 0.0)[0])
        for j in range(0, nplanets):
            ...
            if len(ilist) == 0:
                print(
                    "Error: No albedo file specified for R={0:f} and a={0:f}.".format(
                        R_pl, a_pl
                    )
                )
            ...
            if len(je) == 0:
                ...
                plist = np.concatenate(
                    (
                        np.array(probs[je]) * settings.eecprob,
                        np.array(probs[jn]) * (1.0 - settings.eecprob),
                    )
                )
