...
if filename == "":
    ...
    for root, dirs, files in os.walk(fitsdir):
        for file0 in files:
            if file0[-5:] == ".fits":
                fitslist.append(fitsdir + "/" + file0)
    fitslist = sorted(fitslist)

    for i in range(0, len(fitslist)):
        print(i, fitslist[i])  # print the list with indices if you need it
...
if abs(hdul[-1].header["A"] / np.sqrt(hdul[hstar].header["LSTAR"]) - 1) < 1.0e-5:
    nplanets -= 1  # remove the extra Earth twin if it is present
...
for j in range(1, nplanets):
    ...
    fout.write(
        "{0:f}\t{1:f}\t{2:f}\t{3:f}\t{4:f}\t{5:f}\t{6:f}\t{7:f}\t{8:s}\n".format(
            M, R, a, e, i, longnode, argperi, meananom, albedo
        )
    )

fout.write("Disks\n")
fout.write(
    "nzodis\t\tr\t\tdror\t\trinner\t\thor\t\tg0\t\tg1\t\tg2\t\tw0\t\tw1\t\tw2\t\tcomposition\n"
)
for i in range(0, hdul[2].header["NCOMP"]):
    ...
    fout.write(
        "{0:f}\t{1:f}\t{2:f}\t{3:f}\t{4:f}\t{5:f}\t{6:f}\t{7:f}\t{8:f}\t{9:f}\t{10:f}\t{11:s}\n".format(
            nzodis, r, dror, rinner, hor, g0, g1, g2, w0, w1, w2, composition
        )
    )
